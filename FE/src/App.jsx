    ...
    NAVIGATION.LISTS_MENU_NAVIGATION.forEach((itemMenu) => {
        itemMenu.data.forEach((item) => {
            if (item.subItems.length > 0) {
                item.subItems.forEach((subItem, index) => {
                    if (index === 0) {
                        listRouteNavigation.push({
                            ...
                            component: listComponent.find((item_component) => item_component.name === subItem.component).component,
                        });
                    } else {
                        listRouteNavigation.push({
                            path: item.path + subItem.path,
                ...
                listRouteNavigation.push({
                    path: item.path,
                    component: listComponent.find((item_component) => item_component.name === item.component).component,
                });
            }
        });
    });
