  ...
  const handleOnClick = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_REACT_APP_END_POINT}/verify-otp?email=${guest[0]}&otp=${code}`
      );
  ...
  return (
    ...
    >
      <p className="mt-5 mb-1.5" style={{ color: "rgba(0,0,0,0.5)" }}>
        We sent an email to{" "}
        <span style={{ color: "rgba(0,0,0,0.8)" }}>{guest[0]}</span>. Please
      ...
      >
        ...
        <Button text="Verify" handleOnClick={handleOnClick} isLoading={isLoading}/>
      </Form>
    </div>
  );
};
