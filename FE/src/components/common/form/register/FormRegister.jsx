  ...
  const handleOnClick = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_REACT_APP_END_POINT}/send-otp?email=${email}`
      );
      if (!response || response.status !== 200) throw new Error();
      setGuestRegisterInformation(btoa([email, password, name]));
      navigate("verify-otp")
    } catch (error) {
  ...
  return (
    <>
      ...
      >
        ...
        <Button text="Sign up" handleOnClick={handleOnClick} isLoading={isLoading} />
      </Form>
      <div
        className={
